...
opportunities_tasks = [
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
          type="custom",
          score_fn=lambda x: llm_judge_eval_fn(x, "There should be a new opportunity with Frost, Simmons and Blackwell account for $45,000 in the Qualification stage with a 30% probability, assigned to Jason Doyle with a close date of May 15, 2025."),
        )
    ),
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
          type="custom",
          score_fn=lambda x: llm_judge_eval_fn(x, "The 'Function-based zero-defect standardization' opportunity with Sanchez-Gardner should be updated from Qualification stage to Needs Analysis stage and the probability should be increased from 30% to 50%."),
        )
    ),
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
          type="custom",
          score_fn=lambda x: llm_judge_eval_fn(x, "The 'Quality-focused bandwidth-monitored knowledge user' opportunity with Miller, Mason and Harris should have a new contact Thomas Wilson added as an influencer and the amount should be changed from $303,248.66 to $325,000."),
        )
    )
]
...
contacts_and_accounts_tasks = [
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
          type="custom",
          score_fn=lambda x: llm_judge_eval_fn(x, "Contact Diana Huynh's role should be updated from 'Technical Contact' to 'Decision Maker' for the Peterson LLC account and a follow-up meeting should be scheduled for April 24, 2025 at 10:00 AM."),
        )
    ),
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
          type="custom",
          score_fn=lambda x: llm_judge_eval_fn(x, "There should be a new contact named Maria Rodriguez associated with the Green Ltd account, with role 'Technical Contact' and email address maria.rodriguez@cruz.net."),
        )
    ),
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
          type="custom",
          score_fn=lambda x: llm_judge_eval_fn(x, "There should be a new account 'Technovate Solutions' with type 'Prospect', industry 'Technology', assigned to Cynthia Cuevas, with a note that they were referred by the Wagner and Sons account."),
        )
    )
]
...
leads_tasks = [
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
          type="custom",
          score_fn=lambda x: llm_judge_eval_fn(x, "Lead Michael Diaz should be converted from 'New' status to a contact, with a new account called 'Diaz Enterprises' with industry type 'Technology' and both should be assigned to Adam Dudley."),
        )
    ),
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
          type="custom",
          score_fn=lambda x: llm_judge_eval_fn(x, "The status of lead Cynthia Cowan should be changed from 'Assigned' to 'In Process' and a call should be logged noting that she requested a product demo next week."),
        )
    )
]
...
meetings_tasks = [
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
          type="custom",
          score_fn=lambda x: llm_judge_eval_fn(x, "There should be a new meeting with Walter Montgomery to discuss the 'Persevering local forecast' opportunity with Gonzales, George and Guzman on April 23, 2025 at 2:00 PM with Ashley Powell as an attendee."),
        )
    ),
    PlatoTask(
        ...
        eval_config=CustomEvalConfig(
          type="custom",
          score_fn=lambda x: llm_judge_eval_fn(x, "There should be a group meeting titled 'Q2 Pipeline Review' scheduled for April 25, 2025 at 1:00 PM with all users assigned to opportunities in the Proposal/Price Quote stage, marked as a high priority meeting."),
        )
    )
]


all_tasks = opportunities_tasks + contacts_and_accounts_tasks + leads_tasks + meetings_tasks
