...
async def test_login_functionality(
    env: PlatoEnvironment, page, simulator_name: str, test_id: str
) -> tuple[bool, bool]:
    """
    ...
    try:
        ...
        await page.screenshot(
            path=str(SCREENSHOTS_DIR / f"{simulator_name}_{test_id}_initial.png")
        )
        ...
        print(
            f"Testing login functionality for {simulator_name} using built-in login method"
        )
        ...
        try:
            ...
            await env.login(
                page,
                screenshots_dir=Path(SCREENSHOTS_DIR / f"{simulator_name}_{test_id}"),
            ...
            await page.screenshot(
                path=str(
                    SCREENSHOTS_DIR / f"{simulator_name}_{test_id}_after_login.png"
                )
            )
            ...
            await page.screenshot(
                path=str(
                    SCREENSHOTS_DIR / f"{simulator_name}_{test_id}_login_failed.png"
                )
            )
...
async def run_single_simulator_test(simulator: Dict[str, Any]) -> SimulatorTestMetrics:
    ...
    try:
        ...
        client = Plato(feature_flags={"deployment_color": "green"})
        ...
        try:
            ...
            async with async_playwright() as p:
                ...
                final_screenshot_path = str(
                    FINAL_SCREENSHOTS_DIR / f"{simulator_name}_{env.id}_final.png"
                )
...
async def test_all_simulators(simulator_name: Optional[str] = None):
    ...
    try:
        ...
        if simulator_name:
            matching_simulators = [
                s for s in simulators if s["name"].lower() == simulator_name.lower()
            ]
        ...
        for i, result in enumerate(metrics_results):
            if isinstance(result, Exception):
                print(
                    f"❌ Test failed for {simulators[i]['name']} with exception: {result}"
                )
        ...
        aggregate_metrics = {
            "test_type": "single_simulator_test"
            if simulator_name
            else "comprehensive_simulator_test",
        ...
        metrics_file = (
            METRICS_DIR / f"{filename_prefix}simulator_autologin_test_{timestamp}.json"
        )
        ...
        print(
            f"Overall Success Rate: {(successful_tests / len(simulators)) * 100:.2f}%"
        )
        ...
        if login_attempts > 0:
            print(
                f"Login Success Rate: {(successful_logins / login_attempts) * 100:.2f}%"
            )
        ...
        for metric in all_metrics:
            ...
            print(
                f"  {status} {metric['simulator_name']} ({metric['total_duration']:.2f}s){login_status}"
            )
